-- FIXME (meiersi): add comments; at least a module comment. The most
-- motivating way to add comments is to work backwards from the Haddocks
-- generated by 'cabal haddock' and convert them into accessible
-- documentation.
...
data Message = Message
  { crc     :: !Crc
  , payload :: Payload
    -- FIXME (meiersi): explain why this is non-strict.
  } deriving (Show, Eq)
...
data Request = ProduceRequest
  ...
  , rqPrTopics          :: [RqTopic]   -- FIXME (meiersi): why lazy?
  }
  | MetadataRequest
  { rqMdNumTopics       :: !ListLength
  , rqMdTopicNames      :: [RqTopicName]   -- FIXME (meiersi): why lazy?
  }
  | FetchRequest
...
data RqTopic = RqTopic
  -- FIXME (meiersi): make alignment consistent.
...
data Partition =
  -- FIXME (meiersi): use haddock syntax to document constructors:
